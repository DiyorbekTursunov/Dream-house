document.addEventListener("DOMContentLoaded", function () {
  const e = document.querySelectorAll(".registerBtn"),
    ...
    c = document.getElementById("selectedCountry"),
    r = document.getElementById("selectedCountryCode"),
    s = document.getElementById("countryDropdown"),
    h = document.getElementById("dropdownIcon"),
    m = [
  ...
  let g = "+998";
  function y() {
    (t.style.display = "none"), (document.body.style.overflowY = "scroll");
  }
  c.addEventListener("click", function () {
    "block" === s.style.display
      ? ((s.style.display = "none"),
        (h.innerHTML = '<polyline points="6 9 12 15 18 9"></polyline>'))
      : ((s.innerHTML = ""),
        m.forEach(function (e) {
          const t = document.createElement("div");
          (t.className = "country-option"),
            e.code === g && t.classList.add("selected"),
            (t.innerHTML = `\n          <span>${
              e.name
        ...
        (h.innerHTML = '<polyline points="18 15 12 9 6 15"></polyline>'));
  }),
    ...
    i.addEventListener("input", function (e) {
      const t = e.target.value.replace(/\D/g, ""),
        n = (u[g] || u["+998"]).format(t);
      (i.value = n), (a.style.display = "none");
    }),
    e.forEach(function (e) {
      e.addEventListener("click", function () {
        (t.style.display = "block"), (document.body.style.overflowY = "hidden");
      });
    }),
    ...
    o.addEventListener("submit", function (e) {
      e.preventDefault();
      const t = i.value;
      if (!(u[g] || u["+998"]).validate(t))
        return void (a.style.display = "block");
      (a.style.display = "none"),
        (d.textContent = "YUBORILMOQDA..."),
        (d.disabled = !0);
      const n = new Date(),
        l = n.toLocaleDateString("uz-UZ"),
        o = n.toLocaleTimeString("uz-UZ"),
        c = { TelefonRaqam: g + " " + t, SanaSoat: l + " - " + o };
      localStorage.setItem("formData", JSON.stringify(c)),
        (window.location.href = "/thankYou.html"),
        (d.textContent = "DAVOM ETISH"),
        (d.disabled = !1),
        (i.value = ""),
        y();
    });
  ...
  if (p) {
    const e = setInterval(function () {
      if (f <= 0) return clearInterval(e);
      f--;
      const t = Math.floor(f / 60),
        n = f % 60,
        l = t.toString().padStart(1, "0") + ":" + n.toString().padStart(2, "0");
      p.textContent = l;
    }, 1e3);
  }
});
